...
const DetallesEnvio: React.FC<DetallesEnvioProps> = ({
  ...
  const handleContinue = () => {
    // Opcional: validar los campos si es necesario
    if (!formData.puertoOrigen) {
      alert('Por favor seleccione un puerto de origen');
      return;
    }
    
    if (!formData.puertoDestino) {
      alert('Por favor seleccione un puerto de destino');
      return;
    }
    
    // Continuar al siguiente paso sin llamar al webhook
